...
While Fix has knowledge of cloud-specific resource kinds like [`aws_ec2_volume`](./aws.mdx#aws_ec2_volume) and `gcp_disk`, both of those kinds inherit from the base `volume` kind, which in turn inherits from the `resource` kind:
