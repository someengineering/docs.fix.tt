...
This query will select the aws account with name `sunshine` and then select all nodes outbound to this node. This will select everything Fix knows about nodes in this account.
...
Fix currently supports two types of edges.
...
A volume or network interface might be attached to a volume. The dependency between the 2 is expressed as default edge. Every resource in Fix is connected via an edge of type default.
