...
Most cloud resources are maintained in an account. Accounts are modeled as [nodes](../inventory-graph/index.mdx#nodes) in Fix.
...
Fix provides the `ancestors` and `descendants` section as part of every resource to make this a little easier.
...
- It starts with a `/`. Fix interprets property paths relative to the `reported` section by default. Since we want to access a property on the object root level, we need to prefix the path with a `/`.
...
Fix provides the following information as part of every resource node.
