...
**Fix includes a powerful search engine.**
...
Fix allows for [merging nodes](./merging-nodes.mdx), which can be a useful in case the information is not part of the node itself but in a reachable node nearby. It also allows filtering results with a certain graph structure using the [with clause](./with-clause.mdx).
