...
## Selecting Nodes by [Kind](../inventory-graph/index.mdx#resource-kinds) \{#selecting-nodes-by-kind}

```bash title="Filter by kind"
is(instance)
```

In order to select [nodes](../inventory-graph/index.mdx#nodes) by a specific type, the search syntax supports the `is(<kind>)` function. The term `is(instance)` would select the EC2 instance above, but also all other instances, e.g. Google Cloud instances, while the term `is(aws_ec2_instance)` would select only EC2 instances from AWS.
...
Fix will add a synthetic `id` property to each [node](../inventory-graph/index.mdx#nodes) that is globally unique, no matter which cloud provider you are using. Nodes can be selected by their internal ID via the `id(xyz)` function.
...
In order to filter for specific attributes of a [node](../inventory-graph/index.mdx#nodes), it is possible to define predicates. A predicate always has the syntax `<property_path> <operation> <value>` (e.g., `answer != 42`).
...
is(instance) and /ancestors.account.reported.name=~engineering
```

```bash title="Select all instance nodes wich are not in the engineering account"
is(instance) and not /ancestors.account.reported.name==engineering
```
...
group_ip_permissions[*].from_port>=22 and group_ip_permissions[*].to_port<=22 and group_ip_permissions[*].ip_ranges[*].cidr_ip~"0.0.0.0"
```
...
group_ip_permissions[*].{from_port>=22, to_port<=22, ip_ranges[*].cidr_ip~"0.0.0.0"}
```

:::

Fix supports context notation, which allows us to reference a single position in all predicates. Context notation combines several predicates into a single term which is matched to a nested structure. The path to this structure is defined by a path which can contain arrays that use the wildcard index `[*]`.
...
group_ip_permissions[*].{from_port>=22, to_port<=22, ip_ranges[*].{cidr_ip~"0.0.0.0" or description~"Office access"}}
```
...
Every resource in Fix is represented by a JSON document with the following structure:
...
cpu_count > 3 and /ancestors.account.reported.name=="123"
```
...
is(load_balancer) and in_subnet(public_ip_address, "167.123.0.0/16")
```
