...
The same resource can adhere to multiple resource kinds. This makes it possible to allow to view the same resource in more than one way. For example, an AWS EC2 volume can be viewed as [`aws_ec2_volume`](../resources/aws/aws_ec2_volume.mdx), as compute [`volume`](../resources/base-kinds/volume.mdx), as `aws_resource` and as [`resource`](../resources/index.mdx#resource-base-kind).
